...
### 🌙 界面主题：流光 · 月砚
- **夜幕深空为纸，流光为墨**：玄夜底色上，青冥 → 澜紫 → 霞粉的光带缓慢游走，玻璃面板承光、发丝线描边
- **全站月夜氛围层**（`aurora.js`）：WebGL 满月悬空、薄云掠面、星子呼吸明灭——半分辨率渲染 + 标签页隐藏即暂停，只做氛围不抢正文对比度
- **网关沉浸页**（`inkflow.js`）：Three.js 大满月 + 桂花雨粒子 + 鼠标扰动流光
