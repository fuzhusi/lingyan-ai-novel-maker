...
- **篇章 AI 痕迹检测**：10 项对照语料验证的确定性检测 + 人味分，修正指令自动注入生成链路二次定向修正
- **文风锚例**：真人原文直插 prompt 做风格锚定，覆盖长篇 + 短篇全部正文生成链路；设置页粘贴、写作页一键开关
...
- **稿纸令牌影射**：容器内 CSS 变量自动解析为纸上深调，子规则与模板零改动适配；写作区为真稿纸（界格线 + 白瓷底控件 + 朱砂聚焦）
