...
- **质量门禁 (skill_gate)：** 生成完成后对全文做确定性校验（正则/统计，零 LLM 成本），按当前激活技能选择性检查——对话修饰语、三连排比、首先其次模板、段末升华、直述情绪标签、抽象感官词；违规带原文摘录，前端在生成结果下方渲染报告；API \POST /api/skills/gate-check\ {text}
