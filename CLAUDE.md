...
- **断点续跑（韧性预案）**：每完成一个节点持久化 `outline_nodes`（含 `content`），续写永远从第一个 pending 点接上；页面加载探测到 pending 节点自动亮出「继续生成」（断网/宕机后刷新即可续跑，不逼用户用破坏性的「重新创作」）；网络中断与主动暂停同语义收尾——残片截断到最后一个完整节点；服务端状态保险丝（`try/yield from/finally`）保证任何退出路径（含客户端断开的 GeneratorExit）status 都不停留 `generating`；单节点 LLM 调用失败自动重试 3 次（1s/2s 指数退避），扛住网络抖动
