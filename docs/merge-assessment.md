...
| A4 | ✅ **已实现（2026-09-04，随 Agent 协同 P2）**。**知识抽取「预览-确认」人审环节**：实体/事实/关系自动抽取后先进待确认队列，人核验才写回真源 | NovelForge | ~~空白~~ | `PendingExtraction` 表 + `extraction_queue.py` 服务 + truths/extract `queue=1` 模式 + CLI `queue` 命令组；时序真相已接入，causal_chain 可复用 | M |
...
5. ~~A4 抽取人审队列~~ ✅ 已实现（时序真相已接入 `queue=1` + CLI `queue` 命令组）
