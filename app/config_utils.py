...
def _resolve_llm_model(key_value):
    ...
    if provider and provider.enabled and provider.api_key:
        # 已入库但被取消勾选 = 用户否决该模型（回退自动默认）
        row = LLMModel.query.filter_by(provider_id=pid, model_id=mid).first()
